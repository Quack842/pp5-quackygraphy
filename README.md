...
| When posting, if the user spam the "create" button, there will be multiple logs of the same post. | I fixed this bug by writing code to the handleSubmit try,catch block. I added the disable function when the submition is true, and then a finally block to set the button back to false (Not submitted) and added this code to the button. | N/B
